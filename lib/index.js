...
module.exports = class JmlParser {
  ...
  walk (children) {
    ...
    return children.map(child => this.translate(child))
  }
  parseAttributes (attributes) {
    ...
    keys.forEach(key => {
      if (key[0] === ':') { // attribute
        ...
        result[key] = this.parseExpression(bindAttr)
      } else if (key[0] === '@' || key === '*if') { // event or directive if
        result[key] = this.parseExpression(attributes[key])
